...
val_dataset = os.path.join(args.data_dir, 'dry-run')
...
with torch.no_grad():
    ...
    for batch_idx, hazy in enumerate(val_loader):
        ...
        cpath = os.path.join(imsave_dir, str(ls[batch_idx]))
        imwrite(img_list[batch_idx], cpath)
        image = Image.open(cpath)
        imre = image.resize((1845,750))
        imre.save(cpath, 'JPEG')
